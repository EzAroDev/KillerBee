...
export default function IngredientTable() {
  ...
  const [models, setModels] = useState<Ingredient[]>([]);
  const [sortBy, setSortBy] = useState<keyof Ingredient>("IdIngredient");
    ...
    const fetchData = async () => {
      try {
        ...
        if (
          ...
          res = await api.get("/rd/ingredient");
        } else if (user?.role === "PROD" || user?.role === "USER") {
          res = await api.get("/prod/ingredient");
        }
  ...
  const handleDelete = async (id: string) => {
    try {
      setIsLoading(true);
      const res = await api.delete(`/rd/ingredient/${id}`);
      if (res.ok) {
        setModels((prev) => prev.filter((m) => m.IdIngredient !== id));
            ...
            <Table className="min-w-[600px]">
              <TableHeader className="rounded-md">
                <TableRow>
                  {[
                    ...
                    <TableHead
                      key={key}
                      onClick={() => handleSort(key as keyof Ingredient)}
              ...
              <TableBody className="bg-[#0b0c0d]">
                {paginatedData.map((model) => (
                  <TableRow
                    key={model.IdIngredient}
                    className="hover:bg-[#141516]/80 transition"
                  >
                    <TableCell>{model.IdIngredient}</TableCell>
                    <TableCell>{model.NomIngredient}</TableCell>
                    <TableCell>{model.DescriptionIngredient}</TableCell>
                    {(user?.role === "RD" || user?.role === "ADMIN") && (
                      <TableCell className="text-right space-x-2">
                        <Link
                          href={`/formulaire-ingredient/${model.IdIngredient}`}
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleDelete(model.IdIngredient)}
                        >
